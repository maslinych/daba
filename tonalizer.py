...
def main():
	...
	aparser = argparse.ArgumentParser(description='Tonalizer or automatic tone marker restoration')
	...
	aparser.add_argument('-c', '--chunkmode', help='Chunking mode specification which is effective only for tone', default=2, type=int)
	...
	if args.learn :
		...
		files1 = glob.iglob("../Tashkeela-arabic-diacritized-text-utf8-0.3/texts.txt/*/*.txt")
		files2 = glob.iglob("../Tashkeela-arabic-diacritized-text-utf8-0.3/texts.txt/*.txt")
		...
		allfiles = u"../Tashkeela-arabic-diacritized-text-utf8-0.3/texts.txt/فضائل مجالس الذكر .txt"
		allfiles = u"../Tashkeela-arabic-diacritized-text-utf8-0.3/texts.txt/تحفة المحتاج في شرح المنهاج.txt"
		...
		for infile in allfiles.split(','):
			if infile :
				...
				sent = []
				for sentence in fileReader.file_to_sentences(infile) :
					for token in sentence :
						tags = token[1]
						sent.append((token[0], tags.encode('utf-8')))
					if len(sent) > 1:
						allsents.append(sent)
						sent = []

		if not args.no_coding :
		...
		for phase in range(num_phases) :
			...
			for sent in train_set :
				if not args.no_coding :
					[tokens, labels] = make_tokens_from_sentence(sent, not args.no_coding)
					features = make_features_from_tokens(tokens, phase, not args.no_coding)
					labels = get_sub_tone_code_of_sentence(sent, phase, sel_en = args.filtering, decomposition_en = not args.no_decomposition)
					labels = list(itertools.chain(*labels))
				else :
					[tokens, labels] = make_tokens_from_sentence(sent, not args.no_coding)
					features = make_features_from_tokens(tokens, 0, not args.no_coding)
		...
		if not args.no_coding :
			...
			for phase in range(num_phases) :
				...
				for p, sent in enumerate(gold_set) :
					...
					[tokens, gold_labels] = make_tokens_from_sentence(sent, not args.no_coding)
					features = make_features_from_tokens(tokens, phase, not args.no_coding)
		...
		print "Accuracy : {:>5.3f}".format(accuray2(gold_set, predicted_set, not args.no_coding))

		if args.store :
			stored_filename = args.store
			csv_export(stored_filename, gold_set, predicted_set, not args.no_coding)
